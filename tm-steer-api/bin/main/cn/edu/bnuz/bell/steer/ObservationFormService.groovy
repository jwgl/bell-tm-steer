...
class ObservationFormService {
    ...
    def list(String userId, Integer termId) {
        ...
        def isAdmin = observerSettingService.isAdmin()
        def result
        if (securityService.hasRole("ROLE_DEAN_OF_TEACHING")) {
            def dept = Teacher.load(securityService.userId).department.name
            result = listForDean(termId, dept)
...
<<<<<<< HEAD:tm-steer-api/grails-app/services/cn/edu/bnuz/steer/ObservationFormService.groovy
...
=======
        ...
        if (securityService.hasRole("ROLE_OBSERVER_CAPTAIN")) {
            result = result.grep{
                it.observerType == 1
>>>>>>> 16f239b42c2226047334641d65f58d3770421bd0:tm-steer-api/grails-app/services/cn/edu/bnuz/bell/steer/ObservationFormService.groovy
            }
        }
